...
while ($registro = $resultado->fetch_assoc()) {
    ...
    <tr class='linha-expansivel' data-target="<?= $conteudoExtraID ?>">
        <td>
            <button type='button' class='btn btn-primary btnExpand' data-num="<?= $num ?>"
                data-target="<?= $conteudoExtraID ?>">
                <i class="bi bi-plus icon-expand"></i>
            </button>
        </td>
        <td><?= $num ?></td>
        <td><?= $data ?></td>
    ...
    <tr class='conteudo-extra <?= $conteudoExtraID ?>'>
        <td colspan="10">
            <div class="bg-dark">
                <table id="resultTable1" class="table table-responsive table-hover text-bg-light align-middle">
                    <thead>
                        <tr>
                            <th>Item</th>
                            <th>Código</th>
